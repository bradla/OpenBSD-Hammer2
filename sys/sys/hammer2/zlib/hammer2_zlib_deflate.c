...
int deflateInit2_(z_streamp strm, int level, int method, int windowBits,
					...
					int stream_size);
int deflateResetH2 (z_streamp strm);
	...
	int memLevel, int strategy, const char *version, int stream_size)
{
    ...
    s = (deflate_state *) malloc(sizeof(*s), C_ZLIB_BUFFER_DEFLATE, M_INTWAIT);
    ...
    s->window = (Bytef *) malloc((s->w_size)*2*sizeof(Byte), C_ZLIB_BUFFER_DEFLATE, M_INTWAIT);
    s->prev   = (Posf *)  malloc((s->w_size)*sizeof(Pos), C_ZLIB_BUFFER_DEFLATE, M_INTWAIT);
    s->head   = (Posf *)  malloc((s->hash_size)*sizeof(Pos), C_ZLIB_BUFFER_DEFLATE, M_INTWAIT);

    s->high_water = 0;      /* nothing written to s->window yet */

    s->lit_bufsize = 1 << (memLevel + 6); /* 16K elements by default */

    overlay = (ushf *) malloc((s->lit_bufsize)*(sizeof(ush)+2), C_ZLIB_BUFFER_DEFLATE, M_INTWAIT);
...
deflate_slow(deflate_state *s, int flush)
{
    ...
    for (;;) {
        ...
        if (s->prev_length >= MIN_MATCH && s->match_length <= s->prev_length) {
            ...
            _tr_tally_dist1(s, s->strstart -1 - s->prev_match,
            ...
            _tr_tally_lit1(s, s->window[s->strstart-1], bflush);
            if (bflush) {
...
deflate_rle(deflate_state *s, int flush)
{
    ...
    for (;;) {
        ...
        if (s->match_length >= MIN_MATCH) {
            ...
            _tr_tally_dist1(s, 1, s->match_length - MIN_MATCH, bflush);
            ...
            _tr_tally_lit1 (s, s->window[s->strstart], bflush);
            s->lookahead--;
            s->strstart++;
        }
        if (bflush) FLUSH_BLOCK(s, 0);
